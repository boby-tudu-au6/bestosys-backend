...
module.exports = {
        ...
        usersobj.save((err, doc) => {
            if (err) return next(new AppError(err, 403))
                ...
                transporter.sendMail({
                    ...
                    html: "<h1>hello " + req.body.name + ",here is you activation link</h1><br><a href='http://localhost:3000/activateClient/" + token + ">Activate account</a>" // html body
                }, (err, info) => {
                    if (!err) {
                        return res.status(200).send(
                            {
                                message: "registered successfully",
                                activationLink: "http://localhost:3000/activateClient/" + token,
                                data: doc
                            })
                    } else {
    ...
    forgotPass_Provider: catchAsync(async (req, res, next) => {
        ...
        if (response) {
            ...
            transporter.sendMail({
                ...
                html: "<h1>Click the link to reset you password</h><br>" +
                    "<a href='http://localhost:3000/reset-password/provider/" + token + ">Click here</a>"
            })
            res.send({
                message: "reset link sent to your email",
                link: "http://localhost:3000/reset-password/provider/" + token + ">Click here</a>"
            })
        }
    ...
    forgotPassClient: catchAsync(async (req, res, next) => {
        ...
        if (response) {
            ...
            transporter.sendMail({
                ...
                subject: "Password reset link",
                html: "<h1>Click the link to reset you password</h><br>" +
                    "<a href='http://localhost:3000/reset-password/client/" + token + ">Click here</a>"
            })
            res.send({
                message: "reset link sent to your email",
                link: "http://localhost:3000/reset-password/client/" + token + ">Click here</a>"
            })
        }
        return next(new AppError("The email id enter is invalid", 404))
    }),
            ...
            providersobj.save((err, doc) => {
                if (err) return next(new AppError(err, 402))
                    ...
                    transporter.sendMail({
                        ...
                        to: req.body.email,
                        subject: "Account activation link",
                        html: "<h1>hello " + req.body.name + ",here is you activation link</h1><br><a href='http://localhost:3000/activate/" + token + ">Activate account</a>" // html body
                    }, (err, info) => {
                        if (!err) {
                            return res.status(200).send(
                                {
                                    message: "registered successfully",
                                    activationLink: "http://localhost:3000/activate/" + token,
                                    data: doc
                                })
                        } else {
    ...
    loginprovider: catchAsync(async (req, res, next) => {
        ...
        if (!provider) next(new AppError("Email id is not registered", 403))
